...
let app = new Vue({
        ...
        methods: {
            ...
            themeInit(caseId) {
                 ...
                 * }
                 * */
                $.get("http://uding.com/store/api/case_base_info", {case_id: caseId}, (e) => {
                    // finish
                    const selectAll = []
            ...
            initControlFlow() {
                const optionAll = []
                $.ajax({
                    url: "http://uding.com/store/api/items", type: "GET", success: (res) => {
                        /**
                         * res.data:res[]
                         * interface res {
            ...
            open(index) {
                ...
                console.log('open', this.$refs[`openContent${index}`][0].scrollHeight)
            ...
            upload() {
                ...
                if (this.WhoNew) {
                    this.ArrangeTemporaryStorage.forEach(item => {
                        curList.photoAndVideoList.push({isImg: item.type, src: item.url, id: item.asset_id})
                    })
                } else {
                    this.ArrangeTemporaryStorage.forEach(item => {
                        curList.designPhoto.push({isImg: item.type, src: item.url, id: item.asset_id})
                    })
                }
                ...
                try {
                    ...
                    if (bool && usrOperation) {
                        ...
                        if (!curList.photoAndVideoList[curList.curPhotoVideoNum].isImg) {
                            this.deleteFile(2, curList.photoAndVideoList[curList.curPhotoVideoNum].src, (e) => {
                        ...
                        this.deleteFile(1, curList.photoAndVideoList[curList.curPhotoVideoNum].src, (e) => {
                            curList.photoAndVideoList.splice(curList.curPhotoVideoNum, 1)
                            alert('删除成功！')
                        }, (e) => {
                            alert("删除失败，请重试")
                    ...
                    if (!bool && usrOperation) {
                        if (!curList.designPhoto[curList.curDesignNum].isImg) {
                            this.deleteFile(2, curList.designPhoto[curList.curDesignNum].src, (e) => {
                                alert('删除成功！')
                        ...
                        this.deleteFile(1, curList.designPhoto[curList.curDesignNum].src, (e) => {
                            alert('删除成功！')
                            curList.designPhoto.splice(curList.curDesignNum, 1)
                        }, (e) => {
                            alert('删除失败！请重试')
            ...
            selectTwoChange(e) {
                ...
                let curList = this.offersList[target.data('group')].offers[target.data('son')]
                curList.single = target.val()
                const filterOption = this.AllOptions.filter(item => item.title === curList.single)[0]
                ...
                onBeforeUpload: (e) => {
                    ...
                    for (const eKey in e) {
                        if (this.fileType(1, e[eKey].type)) {
                            this.addFile(1, e[eKey].data, (imgData) => {
                                imgData.data.forEach(item => {
                                    this.assetIds.push(item.asset_id)
                                    this.ArrangeTemporaryStorage.push(Object.assign(item, {type: true})) // type:true 为图片，false 为视频
                            ...
                            this.addFile(2, e[eKey].data, (data) => {
                                console.log(data, 'send file')
                                progressBar.push(true)
                                data.data.forEach(item => {
                                    this.assetIds.push(item.asset_id)
                                    this.ArrangeTemporaryStorage.push(Object.assign(item, {type: false}))
                                    this.temporaryStorage.push(item)
                                })
                                this.upload()
                                this.changeLogo()
